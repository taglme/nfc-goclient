...
func (s *runService) GetAll(adapterID string) ([]models.JobRun, error) {
...
func (s *runService) GetFiltered(adapterID string, filter RunFilter) (runs []models.JobRun, err error) {
	targetUrl := s.url + s.basePath + "/" + adapterID + s.path + buildRunsQueryParams(filter)
	resp, err := s.client.Get(targetUrl)
	if err != nil {
		return runs, errors.Wrap(err, "Can't get runs\n")
	}

	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return runs, errors.Wrap(err, "Can't convert runs to byte slice\n")
	}

	err = handleHttpResponseCode(resp.StatusCode, body)
	if err != nil {
		return runs, errors.Wrap(err, "Error in fetching runs\n")
	}

	var rListResource models.JobRunListResource
	err = json.Unmarshal(body, &rListResource)
	if err != nil {
		return runs, errors.Wrap(err, "Can't unmarshal runs response\n")
	}
